...
def bar_chart(income_sums, expense_sums):
    ...
    ax.bar(index[:len(income_sums)], list(income_sums.values()), bar_width, label='Income')
    ax.bar(index[len(income_sums):], list(expense_sums.values()), bar_width, label='Expenses')
