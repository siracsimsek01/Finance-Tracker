...
class Dashboard(Toplevel):
    def __init__(self, parent):
        ...
        self.transactions_frame = Frame(
            self.main_frame, bg=COLOR_PRIMARY, width=800, height=600
        )
        self.transactions_frame.pack(
            side="left", fill="both", expand=False, padx=10, pady=10, anchor="w"
        )

        self.transactions_label = Label(
            self.main_frame, bg=COLOR_BG, text="Filter Transactions", font=TEXT_HEADING
        )
        ...
        self.filter_by_type = Combobox(
            self.filter_frame, values=["Income", "Expense"], state="readonly", width=10
        )
        self.filter_by_type.set("Type")
        self.filter_by_type.pack(side="left", padx=10, pady=10)

        self.filter_by_category = Combobox(
            self.filter_frame,
            values=[
                "Salary",
                "Pension",
        ...
        self.filter_button = Button(
            self.filter_frame, text="Filter", command=self.filter_transactions
        )
        ...
        self.print_button = Button(
            ...
            width=30,
            height=3,
            command=self.print_transactions,
        )
        self.print_button.place(x=525, y=390)

        self.barChart_btn = Button(
            self.main_frame,
            text="Statistics in Bar Chart",
            width=30,
            height=3,
            command=self.display_barchart,
        )
        self.barChart_btn.place(x=525, y=150)

        self.pieChart_btn = Button(
            self.main_frame,
            text="Statistics in Pie Chart",
            width=30,
            height=3,
            background=COLOR_BG,
            fg=COLOR_BLACK,
            command=self.display_piechart,
        )
        self.pieChart_btn.place(x=525, y=230)

        self.refresh_button = Button(
            self.main_frame, text="Refresh", command=self.refresh_transactions
        )
    ...
    def view_transactions(self):
        ...
        for transaction in transactions:
            ...
            transaction_label = primary_label(
                self.transactions_frame,
                text=f"{category} from {source} ---  {type} \n {date}                                                                  £{amount}",
                font=TEXT_TEXT,
            )
        ...
        try:
            ...
            if time == "Today":
                ...
                filtered_transactions = [
                    t for t in transactions if start_of_week <= t["date"] <= end_of_week
                ]
                ...
                filtered_transactions = [
                    t for t in transactions if start_of_year <= t["date"] <= end_of_year
                ]
            else:  # all Time
            ...
            if selected_type != "Type":
                filtered_transactions = [
                    t for t in filtered_transactions if t["type"] == selected_type
                ]
            ...
            if source != "Enter Source" and source.strip():
                filtered_transactions = [
                    t for t in filtered_transactions if t["payee_source"] == source
                ]
            ...
            if filtered_transactions:
                for transaction in filtered_transactions:
                 ...
                 self.show_no_transactions_label()
    ...
    def display_piechart(self):
        ...
        if chart_type and chart_type.lower() in ["income", "expense"]:
            ...
            tk.messagebox.showerror(
                "Invalid Input", "Please enter either 'Income' or 'Expense'."
            )
    ...
    def print_transactions(self):
        try:
            ...
            messagebox.showinfo(
                "Success", "Transactions printed to data/transactions.txt"
            )
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
    ...
    def show_no_transactions_label(self):
        self.no_transactions_label = primary_label(
        self.transactions_frame,
        text="No transactions to display \n",
        font=TEXT_HEADING,
        bg=COLOR_PRIMARY,
    )
        self.no_transactions_label.pack(side="top", fill="x", padx=10, pady=10)

        for i in range(10, 0, -1):
         self.no_transactions_label.config(text=f"No transactions to display. \n \n Refreshing in {i} seconds...")
         self.transactions_frame.update()
         time.sleep(1)

        self.no_transactions_label.pack_forget()
        self.transactions_frame.after(200, self.refresh_transactions)
